...
def start_bot():
    messagebox.showinfo("اطلاعیه", "ربات در حال اجرا است")
    bot.run()
...
root=Tk()
root.title("استارت ربات روبیکایی")
root.geometry('400x400')
root.configure(bg='#2E4053')
root.iconbitmap('logo.ico')
logo_img = PhotoImage(file="logo.png")
logo_label = Label(root, image=logo_img, bg='#2E4053')
logo_label.pack(pady=10)  

label = Label(root, text="روی دکمه زیر کلیک کنید تا ربات شروع به کار کند", 
                 font=('B Nazanin', 14, 'bold'), fg='white', bg='#2E4053')
label.pack(pady=20)


start_button = Button(root, text='Start Bot', command=start_bot, 
                         font=('B Nazanin', 14, 'bold'), fg='white', 
                         bg='#28B463', activebackground='#1F618D', 
